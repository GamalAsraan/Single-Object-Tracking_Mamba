...
class LTRTrainer(BaseTrainer):
    def __init__(self, actor, loaders, optimizer, settings, lr_scheduler=None, use_amp=False):
        ...
        # Phase 9c bugfix: track which loader ran in current epoch
        self._loaders_run_this_epoch = set()
    ...
    def cycle_dataset(self, loader):
        ...
        max_epochs = getattr(self.settings, '_max_epochs', '?')
        desc = f"Epoch {self.epoch}/{max_epochs} [{loader.name}]"
        pbar = tqdm(enumerate(loader, 1), total=len(loader), desc=desc,
        ...
        for i, data in pbar:
            ...
            if loader.name in self.stats and self.stats[loader.name] is not None:
                for name, val in self.stats[loader.name].items():
                    if hasattr(val, 'avg'):
                        short = name.split('/')[-1]
                        postfix[short] = f"{val.avg:.4f}"
            pbar.set_postfix(postfix)

            if i % self.settings.print_interval == 0 or i == len(loader):
                self._write_log(i, loader, batch_size)
        ...
        summary = f"  ✓ {loader.name} done in {str(datetime.timedelta(seconds=int(epoch_time)))}"
        if loader.name in self.stats and self.stats[loader.name] is not None:
            for name, val in self.stats[loader.name].items():
                if hasattr(val, 'avg'):
                    short = name.split('/')[-1]
                    summary += f"  {short}={val.avg:.4f}"
        print(summary)

    def train_epoch(self):
        # Phase 9c bugfix: reset run-tracking each epoch
        self._loaders_run_this_epoch = set()
        ...
        for loader in self.loaders:
            if self.epoch % loader.epoch_interval == 0:
                ...
                self._loaders_run_this_epoch.add(loader.name)  # mark as run

        self._capture_val_loss()
        self._stats_new_epoch()
        if self.settings.local_rank in [-1, 0]:
            self._write_tensorboard()

    def _capture_val_loss(self):
        """Phase 9c bugfix: only capture val loss if val ACTUALLY ran this epoch.
        Previously read stale .avg=0 from reset AverageMeter."""
        if 'val' not in self._loaders_run_this_epoch:
            self._last_val_loss = None
            return
        val_stats = self.stats.get('val', None)
        if val_stats is None:
            self._last_val_loss = None
            return
        loss_meter = val_stats.get('Loss/total', None)
        if loss_meter is None or not hasattr(loss_meter, 'avg'):
            self._last_val_loss = None
            return
        if loss_meter.avg <= 0.0:  # sanity: real losses are always > 0
            print(f"⚠️  Refusing to capture suspicious val_loss={loss_meter.avg}")
            self._last_val_loss = None
            return
        self._last_val_loss = loss_meter.avg
    ...
    def _write_log(self, i, loader, batch_size):
        ...
        log_str = '[%s: %d, %d / %d] ' % (loader.name, self.epoch, i, len(loader))
        log_str += 'FPS: %.1f (%.1f)  ,  ' % (average_fps, batch_fps)
        log_str += 'DataTime: %.3f (%.3f)  ,  ' % (
            self.avg_date_time / self.num_frames * batch_size,
            self.avg_gpu_trans_time / self.num_frames * batch_size)
        log_str += 'ForwardTime: %.3f  ,  ' % (self.avg_forward_time / self.num_frames * batch_size)
