...
# Visdom is an optional debug-only dependency. On Kaggle / minimal envs it's
# not installed. Defer the import so trackers can be used without it.
try:
...
class BaseTracker:
    ...
    def _init_visdom(self, visdom_info, debug):
        ...
        if debug > 0 and visdom_info.get('use_visdom', True):
            if not _VISDOM_AVAILABLE:
                print("!!! WARNING: visdom is not installed; debug visualization disabled !!!")
                return
            try:
